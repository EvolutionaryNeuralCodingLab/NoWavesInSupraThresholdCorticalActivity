...
dff1 = np.load('Sup Fig 7 Example.npy')
dff1_L = dff1

brain_mask = np.load('brain_mask_64.npy')
brain_mask_left = brain_mask[:, :32]
...
def classify_jacobian_patterns(flow, dff1, brain_mask,
                               plane_threshold=0.8,
                               min_radius=4,
                               min_duration=2,
                               Nv=8,
    ...
    """
    N, M, T, _ = flow.shape
    delta_1 = angular_alpha * 2 * np.pi / Nv * 3
...
frame_labels_left, raw_detections_left = classify_jacobian_patterns(flow_left, dff1_L,brain_mask_left)
...
brain_mask = np.load('brain_mask_64.npy')
brain_mask_right = brain_mask[:, 32:]
...
frame_labels_right, raw_detections_right = classify_jacobian_patterns(flow_right, dff1_R,brain_mask_right)
...
def plot_both_hemispheres_frames_and_raster(
    ...
    assert len(frames) == 6, \
        "Please provide exactly 6 frame numbers."

    H, W = data_left.shape[:2]

    assert data_right.shape[:2] == (H, W), \
        "Left/right shapes must match."
    ...
    # Sum detections from hemispheres
    ...
    gs = gridspec.GridSpec( 2, 1, height_ratios=[ 1, 0.8 ], hspace=0.32  )

    top_gs = gs[0].subgridspec( 1, 6, wspace=0.03 )
    ...
    for k, frame in enumerate(frames):
        ...
        ax = fig.add_subplot( top_gs[0, k] )

        plot_mask_left = np.flipud(brain_mask_left)
        plot_mask_right = np.flipud(brain_mask_right)

        left_img = (  data_left[:,:,frame].copy() )
        left_img[plot_mask_left == 0] = 0
        left_flow = (flow_left[:,:,frame,:].copy() )
        left_flow[plot_mask_left == 0 ] = 0

        right_img = (data_right[:,:,frame ].copy())
        right_img[plot_mask_right == 0 ] = 0
        right_flow = (flow_right[:,:,frame,:].copy())
        right_flow[plot_mask_right == 0] = 0
        ...
        # Plane ONLY if both hemispheres
        if ( pattern_left[0,frame] > 0 and pattern_right[0,frame] > 0):
            title_parts.append("Plane")

        if ( pattern_total[4,frame] > 0 ):
            title_parts.append("Standing")

        ax.set_title(" + ".join(title_parts),fontsize=7,pad=2)
        ax.set_xlim(0, 2 * W)
        ax.set_ylim(H, 0 )
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    bottom_gs = gs[1].subgridspec( 1, 2, width_ratios=[1, 1], wspace=0.40)

    ax_raster = fig.add_subplot( bottom_gs[0,0] )
    ax_alpha = fig.add_subplot( bottom_gs[0,1] )

    raster = (pattern_total.copy())

    # Plane wave only when BOTH hemispheres detected it
    raster[0, :] = (
        raster[0, :] == 2
    ).astype(int)
    ...
    ax_raster.set_xlabel( "Time [sec]", fontsize=8)
    ax_raster.set_xticks( np.arange( 0, np.floor(duration_sec)+1, 1 ) )
    ax_raster.tick_params(axis="x",labelsize=7)


    cbar = fig.colorbar( im, ax=ax_raster, fraction=0.025, pad=0.02)
    ...
    ax_alpha.plot( x, waviness_scores, marker="o", markersize=3, linewidth=1.2, label="Waviness score", color='#5f86d6')
    ax_alpha.plot( x, plane_fractions, marker="o", markersize=3, linewidth=1.2, label="Plane-wave frames", color='#876f3e')
    ax_alpha.set_xlabel( r"Smoothness parameter $\alpha$", fontsize=8)
    ax_alpha.set_xticks(x)
    ax_alpha.set_xticklabels([str(a) for a in alpha_values],fontsize=7 )
    ax_alpha.set_ylabel( "Fraction", fontsize=8 )
    ax_alpha.set_ylim(0, 1 )
    ax_alpha.set_yticks([0,1])
    ax_alpha.tick_params( axis="y", labelsize=7 )

    ax_alpha.spines['top'].set_visible(False)
    ax_alpha.spines['right'].set_visible(False)
    ax_alpha.legend( frameon=False, fontsize=7, loc="best")


    legend_handles = [

        Line2D([0],[0],
            marker="o", color="w", label="Source",
            markerfacecolor= pattern_colors["Source"],
        ...
        Line2D([0],[0],
            marker="o", color="w", label="Sink",
            markerfacecolor=pattern_colors["Sink"],
            markeredgecolor="white",
            markeredgewidth=0.7,
            markersize=7
        ),
        ...
        Line2D([0],[0],
            marker="o", color="w", label="Saddle",
            markerfacecolor= pattern_colors["Saddle"],
            markeredgecolor= "white",
            markeredgewidth=0.7,
            markersize=7
        )
    ...
    fig.legend( handles=legend_handles, loc="upper left", bbox_to_anchor=( 0.01, 0.99), frameon=True, fontsize=7, ncol=1, borderaxespad=0.2 )

    plt.savefig( filename, bbox_inches="tight", dpi=300 )
    plt.show()


frames = [1, 8, 19, 26, 30, 36]
...
def run_alpha_sensitivity(
    ...
    for hs_alpha in alpha_values:

        print("\n================================")
        print(f"Alpha = {hs_alpha}\n")



        full = PreDataProcessing( dff1.copy() )
        full = FlowAnalyze(full )
        full.horn_schunck_flow(alpha=hs_alpha,num_iter=150,phase=False)
        full.calculate_waveness( type="cortex")

        # Values only in activated pixels
        flattened_values = (full.waveness[:, :, 3][full.mask == 1]).flatten()

        total_above_lim = np.count_nonzero(flattened_values[flattened_values > lim])
        total_non_zero = np.count_nonzero(full.mask)
        ...
        waviness_scores.append(ratio)



        left = PreDataProcessing( dff_left.copy() )
        left = FlowAnalyze(left)
        left.horn_schunck_flow( alpha=hs_alpha, num_iter=500, phase=False)
        flow_left_alpha = ( left.velocities.copy())


        right = PreDataProcessing( dff_right.copy())
        right = FlowAnalyze(right)
        right.horn_schunck_flow(alpha=hs_alpha,num_iter=500,phase=False)
        flow_right_alpha = (right.velocities.copy())


        pattern_left_alpha, _ = ( classify_jacobian_patterns( flow_left_alpha, dff_left, brain_mask_left ) )
        pattern_right_alpha, _ = ( classify_jacobian_patterns( flow_right_alpha, dff_right, brain_mask_right ) )


        bilateral_plane = ( (pattern_left_alpha[0, :] > 0) & (pattern_right_alpha[0, :] > 0) )

        n_plane_frames = np.count_nonzero( bilateral_plane )
        n_total_frames = len( bilateral_plane )
        plane_fraction = ( n_plane_frames / n_total_frames )

        plane_fractions.append(plane_fraction)
    ...
    return (np.array(waviness_scores), np.array(plane_fractions) )




waviness_scores, plane_fractions = (
...
plot_both_hemispheres_frames_and_raster(
    ...
    alpha_values=alpha_values,
    waviness_scores=waviness_scores,
