...
class Display:
    ...
    def plot_frames(self, space, scale):
        ...
        frame_indices = [0,6,15,25,28,36]  ## SUP FIG 7
    ...
    def plot_full_analysis_on_axes( self, ax, ax2, ax3, space, scale, data_type):
        ...
        summed_momentum = self.sum_phase_space[:, :, -1, :].copy()

        zero_mask = ( (summed_momentum[:, :, 0] == 0) & (summed_momentum[:, :, 1] == 0) )

        summed_momentum[zero_mask] = np.nan

        plot_quiver( ax, summed_momentum, spacing=space, scale=0.15, color='black', width=0.006 )
        ...
        # PANEL 3 — WAVINESS MAP

        def cyclic_hsv_cmap(n=256):

            hue = (
                          np.linspace(0, 1, n + 1) + 1 / 3
                  ) % 1

            saturation = (
                    0.7 +
                    0.3 * np.sin(2 * np.pi * hue)
            )

            value = (
                    0.85 +
                    0.15 * np.cos(2 * np.pi * hue)
            )
        ...
        waviness_alpha = data.waveness[:, :, 3].copy()
        waviness_alpha = np.where(waviness_alpha < self.lim,0,waviness_alpha)

        ax3.imshow( data.waveness[:, :, 0], cmap=cmap, vmin=-np.pi, vmax=np.pi, alpha=waviness_alpha)

        ax3.set_aspect('equal')
        ax3.axis("off")

    def plot_frames_with_full_analysis(self, space, scale, data_type):
        ...
        outer_gs = gridspec.GridSpec( 5, 1, height_ratios=[1, 1, 1, 0.20, 1.5], hspace=0.18 )
        frames_gs = gridspec.GridSpecFromSubplotSpec( 3, n_frames,  subplot_spec=outer_gs[0:3], hspace=0.05, wspace=0.03 )

        calcium_axes = [ fig.add_subplot(frames_gs[0, i]) for i in range(n_frames)]
        flow_axes = [ fig.add_subplot(frames_gs[1, i]) for i in range(n_frames)]
        momentum_axes = [ fig.add_subplot(frames_gs[2, i]) for i in range(n_frames) ]


        for i, frame_idx in enumerate(frame_indices):
            # ----- optic flow -----
            flow = self.flows[frame_idx].copy()

            flow[:, :, 0] *= brain_mask
            flow[:, :, 1] *= brain_mask

            # ----- momentum -----
            momentum = self.phase_space[:, :, frame_idx, :].copy()
            ...
            ax = calcium_axes[i]
            ax.imshow( self.dff[:, :, frame_idx], cmap=self.color_map, vmin=0, vmax=1 )
            ...
            plot_quiver( ax, flow, spacing=space, scale=scale, color="black", width=0.006)
            ...
            plot_quiver( ax, momentum, spacing=space, scale=scale * 0.7, color="black", width=0.006 )
        ...
        analysis_gs = gridspec.GridSpecFromSubplotSpec( 1, 5, subplot_spec=outer_gs[4], width_ratios=[0.55, 1, 1, 1, 0.55], wspace=0.30 )

        ax1 = fig.add_subplot(analysis_gs[0, 1])
        ax2 = fig.add_subplot(analysis_gs[0, 2])
        ax3 = fig.add_subplot(analysis_gs[0, 3])

        self.plot_full_analysis_on_axes( ax1, ax2, ax3, space=space, scale=scale, data_type=data_type )

        plt.savefig("Sup_fig_7_upper_part.pdf", bbox_inches="tight", dpi=400)
        plt.show()
...
display.title = 'Sup Fig 7 Example'
display.plot_frames_with_full_analysis( space = 5 , scale = 0.04,data_type='cortex')
